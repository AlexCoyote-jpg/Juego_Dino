...
from core.responsive_scaler import ResponsiveScaler
...
class JuegoEjemplo(JuegoBase):
    def __init__(self, nombre, pantalla, config, dificultad, fondo, navbar, images, sounds, return_to_menu):
        ...
        self.opciones = [42, 24, 12, 36]
        self.init_responsive_ui()
        self._last_mouse_pos = (-1, -1)  # Para evitar cálculos innecesarios

    def update(self, dt=None):
        # Solo actualizar si la ventana está activa para ahorrar recursos
        if pygame.display.get_active():
            super().update(dt)

    def draw(self, surface=None):
        # Usar double buffering implícito de pygame para evitar flickering
        super().draw(surface)
        self.draw_animacion_estrellas()
        self.draw_particulas()
        self.dibujar_feedback()
    ...
    def handle_event(self, evento):
        # Procesar solo eventos relevantes y evitar cálculos innecesarios
        if evento.type == pygame.MOUSEMOTION:
            # Solo actualizar tooltip si el mouse realmente se movió
            if evento.pos != getattr(self, "_last_mouse_pos", (-1, -1)):
                self._last_mouse_pos = evento.pos
                if hasattr(self, 'tooltip_manager'):
                    self.tooltip_manager.update(evento.pos)
            ...
            for i, boton in enumerate(self.opcion_botones):
                if boton.rect.collidepoint(mouse_pos):
                    ...
                    if valor == self.respuesta_correcta:
                        ...
                        self.mostrar_feedback(False, respuesta_correcta=self.respuesta_correcta)
                    # Generar nueva pregunta y opciones para probar
                    self.respuesta_correcta = random.randint(10, 99)
                    self.operacion_actual = f"{self.respuesta_correcta - random.randint(1, 9)} + {random.randint(1, 9)} = ?"
                    self.opciones = self.generar_opciones(self.respuesta_correcta, 4)
                    break
        return super().handle_event(evento)

def test_responsive_game():
    pygame.init()
    pantalla = pygame.display.set_mode((1280, 720), pygame.RESIZABLE | pygame.DOUBLEBUF)
    ...
    while ejecutando:
        ...
        juego.draw()
        pygame.display.flip()
        reloj.tick(90)  # Subir el framerate para mayor fluidez
